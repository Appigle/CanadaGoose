...
if [ "$CI" = "true" ] || [ "$GITHUB_ACTIONS" = "true" ]; then
    echo "🔍 CI/CD environment detected - skipping service startup"
    CI_MODE=true
else
    echo "🔍 Local environment detected - starting all services"
    CI_MODE=false
fi
...
cleanup() {
    ...
    # Only cleanup if not in CI mode
...
# Only start services if not in CI mode
if [ "$CI_MODE" = "false" ]; then
    ...
    export JWT_SECRET=chiputaobutuputaobutuputaopibuchiputaopibuchiputaodaotuputaopi
    ...
    # In CI mode, just activate the virtual environment and install dependencies
    echo "🔧 Activating Python virtual environment..."
    source venv/bin/activate
